...
ER DENNE NOTATION KORREKT? JEG ER I TVIVL OG DET SAMME FOR POISSON MODELLEN

where $\epsilon_i \sim B_i(0,\sigma^2)$. Here $p_{clo,i}$ is the probability of changing clothing level. The residuals are assumed to be independent and identically distributed. The residual standard deviation is estimated to $\hat{\sigma} = 1.13$ and a confidence interval is computed based on theorem 3.5. The estimated parameters and 95\% confidence intervals calculated based on profile likelihood can be seen in table \ref{tab:par_binomial}. 
