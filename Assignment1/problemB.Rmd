...
\textbf{Model 2, eq} $\eqref{eq:prob2_full1}$ &  subjId & 0.480 & 46 & 1.571 & 0.074 \\ 
...
\textbf{Model 2, eq} $\eqref{eq:prob2_full2}$&  subjId & 0.370 & 46 & 0.925 & 0.603 \\ 
...
As in part 1, we remove first higher order terms before we remove lower order terms. We choose which terms to remove by looking at the type 3 deviance table. We start by removing the terms with the highest p-value, and keep removing terms until all terms are significant. Each removed term can be seen in \ref{tab:prob2_deviance}
...
The interpretation of the parameters by using \cref{eq:prob2_full2} instead of \cref{eq:prob2_full1} become very simple. The subject ID estimates is the individual intercepts, while $\beta_1$ is a parameter defines the increase/decrease in clothing level as the indoor temperature increases/decreases. The uncertainty of $\beta_1$ can be found in table \ref{tab:prob2_CI}.
```{r}
m=confint(fit3B2)[1,]
...
$\eqref{eq:prob2_full2}$. The uncertainty is estimated with a Wald type confidence interval.}\label{tab:prob2_CI}
\end{table}
