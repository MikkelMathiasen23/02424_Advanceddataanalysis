...
As in part 1, we remove first higher order terms before we remove lower order terms. We choose which terms to remove by looking at the Type III partitioning in the ANOVA table. We start by removing the terms with the highest p-value, and keep removing terms until all terms are significant. Each removed term can be seen in table \ref{tab:prob2_deviance}
