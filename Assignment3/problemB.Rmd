...
We know from Question 4 that the marginal distribution can be written as a student t-distribution. The marginal distribution can therefore be written as: 

\begin{equation}
L_M(y,\beta,\sigma_u,\sigma) = |\alpha(sex_i)\Sigma^{-1}|\dfrac{\Gamma(\phi+1/2)}{\sqrt{2\pi\phi}\Gamma(\phi)}( 1 + \dfrac{(y-X\beta)^T(y-X\beta)}{2\phi}(\alpha(sex_i)\Sigma^{-1}))^{-\phi-1/2}  
\end{equation}
SKAL DET VÆRE SIGMA^-2????
...
Again, we optimized the variance parameters in the log-domain to ensure positive variance, and we did set $\alpha("male") = 1$ and $\alpha("female") = a$. The estimated log-likelihood was: 1689.8 therefore being higher than the previous model. The covariance structure of $\Sigma$ is still the same as in equation (\ref{eq:Sigma_i}). 

INDSÆT TABEL AF PARAMETRE - PROBLEMER MED NANS FRA HESSIAN 
...
\begin{tabular}{rrrr}
 ...
 & Estimate & Lower 2.5\% & Upper 97.5\% \\ 
  ...
  $\alpha$ & 0.0514 & 0.0145 & 0.1821 \\ 
   \hline
\end{tabular}
\caption{This table shows the estimated parameters and their 95\% (Wald) confidence interval.}
\label{tab:coef_q6}
\end{table}
...
When comparing the model results from question 5 and 6 it is clear that the estimated log-likelihood of the model from question 5 yields the highest log-likelihood which could indicate that this model is best. It should be noted that the difference is: 1689.8-1687.9 = 1.9.

Also it should be noted that the Laplace approximation is an approximation and it therefore is expexted that this would yield a difference from the optimal likelihood. 
