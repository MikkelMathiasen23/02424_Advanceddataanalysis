...
nllQ5<- function(pars,alpha_sex="male"){
  ...
  for(i in unique(dat$subjId)){
    ...
    chol_sig =  chol(sig)
    chol_sig_inv = chol2inv(chol_sig)
    
    test = sqrt(t(y-x%*%beta)%*%chol_sig_inv%*%(y-x%*%beta))
    print(test)
    tmp = dt(x = test,df = 2*exp(pars["phi"]),log=T)
    
    

    tmp2 = det(chol_sig_inv)*tmp
...
library(mvnfast)
dmvt
pars=c("mu"=0.59,"sex_male"=-0.08,"alpha"=2.7,"sigma_sub_day^2"=-5.681,"sigma_s^2"=-6.5,"sigma^2"=-8.08,"phi"=2)
nllQ5(pars)
unique(diag(test))
...
optQ5_2 = optim(pars,fn=nllQ5,alpha_sex="male")
optQ5_2
