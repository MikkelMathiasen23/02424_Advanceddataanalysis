...
Type objective_function<Type>::operator() ()
{
  ...
  for(int i=0; i < nsubj; i++){
    ...
    f -= dnorm(u(i), mean_ran , exp(sigma_u+0.5*alpha_u_sex_male(i)*alpha), true);
  }
  // subject ID and day interaction random effect for v
  for(int j=0; j < nsubj_day; j++){
    // v~N(0,sigma_v^2*alpha(sex_i)*exp(-gamma_i))
    i = subjId_v(j);
    f -= dnorm(v(j),mean_ran,exp(sigma_v+0.5*alpha*alpha_v_sex_male(j)),true);
  ...
  for(int k =0; k < nobs; k++){
    ...
    // res[k] = clo[k]-pred[k];
    f -= dnorm(clo(k),pred(k),exp(sigma+0.5*alpha*sex_male(k)),true);
  }
  return f;
}
