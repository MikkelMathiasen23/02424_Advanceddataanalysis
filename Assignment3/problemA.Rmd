...
From table \ref{tab:coef_1} it should also be noted that the gender of the subject appears have a large impact on the clothing level compared to the other parameters as the intercept and the $sexmale$ coefficient are large. Also it should be noted that the correction term for males $sexmale:tInOp$ on the the clothing level are close to zero and therefore indicating that males are not very different compared to females in this modeling setup.
...
$$
where different autocorrelation-structures for time are tested with $subday$ as a random effect. First, the auto-correlation structure will be investigated where a Auto-Regressive(1), Gaussian and exponential correlation structure will be tested. The Gaussian correlation structure are given by: 

\begin{align}
  \tau^2 \exp{\left(\dfrac{-(t_{i1}-t_{i2})^2}{\phi^2}\right)}
\end{align}
...
\begin{align}
  \tau^2 \exp{\left(\dfrac{-|t_{i1}-t_{i2}|}{\phi^2}\right)}
\end{align}
