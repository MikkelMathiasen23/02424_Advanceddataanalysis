...
$$ clo_i = \beta_0  + \beta_1sex_i +  \beta_2tOut_i +  \beta_3 tInOp_i+ \mathbf{\beta_{4}time_{i}} + \beta_5sex_i(tOut_i) + \beta_6sex_i(tInOp_i) + \mathbf{\beta_{7} day_{i}}+ SubjId_i + \epsilon_{ij}$$
...
$$ clo_i = \beta_0  + \beta_1sex_i +  \beta_2tOut_i +  \beta_3 tInOp_i+ \mathbf{\boldsymbol{\beta}_{4}time_{i}} + \beta_5sex_i(tOut_i) + \beta_6sex_i(tInOp_i) + \mathbf{\boldsymbol{\beta_{7}} day_{i}} + SubjId_i + \epsilon_{ij}$$

where $SubjId_i \sim N(0,\sigma_u^2)$, $\epsilon_{ij} \sim N(0,\sigma^2)$ and both are mutually independent. $sex_i(tOut_i)$ indicates the interaction term between gender of the subject and the Outdoor temperature. The model will be reduced based on backward selection using Type III partitioning where higher order terms are removed first. Firstly, it is determined whether time should be included as a factor. Two models are generated according to above equation where the only difference are whether time is included as a factor or numeric. The log-likelihood of the two models are: 706.5 and 711.6 for the model including time as numeric and factor respectively and thereby indicating that time should be included as a factor.
...
$$ clo_i = \beta_0  + \beta_1sex_i +  \beta_2tOut_i +  \beta_3 tInOp_i+ \mathbf{\beta_{4}time_{i}} + \beta_5sex_i(tInOp_i) + \mathbf{\beta_{6}day_{i}}+ SubjId_i + \epsilon_{ij}$$

Thereby it is only the interaction term between $sex(tOut)$ that are removed.
...
NEJ - VI SÅ I TIDLIGERE OPGAVE DET VAR NØDVENDIGT 
...
From table \ref{tab:coef_1} it should also be noted that it appears that the gender of the subject appears to be of large effect on the clothing level as the intercept and the $sexmale$ coefficient are large. Also it should be noted that the correction term for males $sexmale:tInOp$ on the the clothing level are close to zero and therefore indicating that males are not very different compared to females in this modeling setup.
...
$$ clo_i = \beta_0  + \beta_1sex_i +  \beta_2tOut_i +  \beta_3 tInOp_i+ \mathbf{\beta_{4}time_{i}} + \beta_5sex_i(tInOp_i) + SubjId_i +day_{id} +  \epsilon_{ij}$$


The random effect day is nested within the random effect Subject ID. Here $SubjId_i \sim N(0,\sigma_u^2)$, $\epsilon_{ij} \sim N(0,\sigma^2)$ and $day_{id} \sim N(0, \sigma_d^2)$ and all three are mutually independent. Again $sex_i(tOut_i)$ indicates the interaction term between gender of the subject and the Outdoor temperature. Again time will be included as a factor based on the result in the previous subsection.
...
The fixed effect coefficients are presented in table \ref{tab:coef_2} including 95 \% confidence intervals. It should be noted that the majority of the contribution comes from the gender of the subject ie. the intercept and the sex coefficient. Also it should be noted that that the male correction term of indoor operating temperature $sexmale:tInOp$ almost cancels out the $tInOp$ coefficient thereby indicating that males clothing level does not appear to be affected by the indoor temperature. The outdoor temperature coefficient are close to zero indicating that either females or males are not very affected by the outdoor temperature. The intercept indicating the female effect on the clothing level are higher than the gender contribution for males ($intercept + sexmale$) indicating that males generally has a lower clothing level than males. 

In figure \ref{fig:hist_2} that the day random effects appear to be normal-distributed with some tails. The subject ID random effects appears to have some heavy tails, and this might indicate that modeling these as normal distributed might not be the correct choice. 
...
We try to investigate the residuals splitted into gender see figure \ref{fig:resid_2}, and it is apparent that the model captures the male clothing behavior better than the females. Some extreme points should be noted from the male qq-plot but as seen in figure \ref{fig:part1_exploratory} also one male appear to have a very different behavior than the rest which could be due to some of these extreme points. When looking at the females clearly some of the variance are not accounted for, and this could be accounted for through a weighted residual structure.
...
As can be seen in table \ref{tab:1.51}, all the information criteria points to using a auto-regressive(1) correlation structure. The model will be reduced using type 3 partitioning and backwards selection. Higher order terms will be removed before lower order terms. 
